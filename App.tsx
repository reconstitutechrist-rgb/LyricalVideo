...
const MUSIC_SECTIONS = ['Intro', 'Verse 1', 'Verse 2', 'Verse 3', 'Pre-Chorus', 'Chorus', 'Bridge', 'Outro', 'Instrumental', 'Hook'];
...
const App = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Welcome to Lyrical Flow. I am your AI Director. Upload a track, and lets create something epic.', timestamp: new Date() }
  ]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'model', text: `Analyzed "${metadata.title}" (${metadata.genre}). Lyrics aligned and ready!`, timestamp: new Date() }]);
    } catch (err) {
      console.error(err);
      setChatMessages(prev => [...prev, { role: 'model', text: 'Error analyzing audio. Please try again.', timestamp: new Date() }]);
    } finally {
  ...
  const updateKeyframe = (lyricIndex: number, kfIndex: number, field: keyof TextKeyframe, value: any) => {
  ...
  const addKeyframe = (lyricIndex: number) => {
      ...
      newKeyframes.push({ time: defaultTime, x: 0, y: 0, scale: 1, rotation: 0, opacity: 1, easing: 'linear' });
      updateLyric(lyricIndex, 'keyframes', newKeyframes);
  ...
  const applyBulkTimeShift = () => {
      ...
      const newLyrics = state.lyrics.map((line, idx) => {
          if (selectedLyricIndices.has(idx)) {
              return { ...line, startTime: Number((line.startTime + shift).toFixed(2)), endTime: Number((line.endTime + shift).toFixed(2)) };
          }
          return line;
      });
      setState(prev => ({ ...prev, lyrics: newLyrics }));
  ...
  const applyTextTransform = (type: 'upper' | 'lower' | 'capitalize') => {
      ...
      const newLyrics = state.lyrics.map((line, idx) => {
          if (selectedLyricIndices.has(idx)) {
              ...
              else if (type === 'capitalize') text = text.toLowerCase().replace(/(^\w|\s\w)/g, m => m.toUpperCase());
              return { ...line, text };
          }
          return line;
      });
  ...
  const applyOverride = (type: 'style' | 'palette', value: string | undefined) => {
    ...
    const newLyrics = state.lyrics.map((line, idx) => {
        if (selectedLyricIndices.has(idx)) {
            ...
            if (type === 'palette') return { ...line, paletteOverride: value as ColorPalette | undefined };
        }
        return line;
    });
    setState(prev => ({ ...prev, lyrics: newLyrics }));
  };
  const updateVisualSetting = <K extends keyof AppState['visualSettings']>(key: K, value: AppState['visualSettings'][K]) => {
      setState(prev => ({ ...prev, visualSettings: { ...prev.visualSettings, [key]: value } }));
  };
  const updateFrequencyMapping = (key: keyof AppState['visualSettings']['frequencyMapping'], value: FrequencyBand) => {
      setState(prev => ({ ...prev, visualSettings: { ...prev.visualSettings, frequencyMapping: { ...prev.visualSettings.frequencyMapping, [key]: value } } }));
  };

  const blendModes: BlendMode[] = ['source-over', 'multiply', 'screen', 'overlay', 'darken', 'lighten', 'color-dodge', 'color-burn', 'hard-light', 'soft-light', 'difference', 'exclusion', 'hue', 'saturation', 'color', 'luminosity'];
    ...
    <div className="flex h-screen w-full bg-[#0f172a] text-white overflow-hidden font-sans">
      ...
      <div className="w-80 border-r border-white/10 flex flex-col bg-slate-900/80 backdrop-blur-xl z-10">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-xs text-slate-400 font-medium tracking-widest mt-1 uppercase">AI Studio Pro</p>
        </div>
        ...
        <div className="p-4 space-y-6 overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div className="space-y-3">
             <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">01. Source Material</h3>
             {!state.audioFile && (
               <textarea 
                  ...
                  onChange={(e) => setState(prev => ({...prev, userProvidedLyrics: e.target.value}))}
               />
             )}
             <div className="relative group">
                ...
                <label className="relative flex flex-col items-center justify-center h-20 bg-slate-900 rounded-xl cursor-pointer border border-white/10 hover:bg-slate-800 transition-all">
                  <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-white">{state.audioFile ? "Change Song" : "Upload Audio"}</span>
                  </div>
          ...
          {/* 2. VISUAL DIRECTION */}
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">02. Visual Direction</h3>
            ...
            <div className="flex gap-2">
                 {['9:16', '16:9', '1:1'].map(ar => (
                     <button key={ar} onClick={() => setState(prev => ({...prev, aspectRatio: ar as AspectRatio}))} className={`flex-1 py-1 text-xs rounded border ${state.aspectRatio === ar ? 'border-cyan-500 text-cyan-400 bg-cyan-500/10' : 'border-white/10 text-slate-500'}`}>{ar}</button>
                 ))}
            </div>
          </div>
          ...
          <div className="space-y-3 pt-2 border-t border-white/10">
             <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">03. FX Customization</h3>
             </div>
             
             {/* Color Palette */}
             <div className="flex flex-col gap-1">
                <label className="text-[10px] text-slate-400">Color Palette</label>
                <div className="flex gap-1 flex-wrap">{(['neon', 'sunset', 'ocean', 'matrix', 'fire'] as ColorPalette[]).map(p => (<button key={p} onClick={() => updateVisualSetting('palette', p)} className={`px-2 py-1 text-[10px] rounded border capitalize ${state.visualSettings.palette === p ? 'bg-purple-500/20 border-purple-500 text-purple-200' : 'bg-slate-800 border-white/5 text-slate-400'}`}>{p}</button>))}</div>
             </div>
             ...
             <div className="mt-3 p-2 bg-white/5 rounded border border-white/5 space-y-2">
                 <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Text Styling</span>
                 </div>
                 ...
                 <div className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                        {(['NONE', 'TYPEWRITER', 'FADE_CHARS', 'KINETIC', 'BOUNCE'] as TextAnimationStyle[]).map(anim => (
                            <button 
                                key={anim} 
             ...
             <div className="mt-2 space-y-2">
                <div><div className="flex justify-between text-[10px] text-slate-400 mb-1"><span>Animation Speed</span><span>{state.visualSettings.particleSpeed}x</span></div><input type="range" min="0.1" max="3" step="0.1" value={state.visualSettings.particleSpeed} onChange={(e) => updateVisualSetting('particleSpeed', parseFloat(e.target.value))} className="w-full h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-purple-500"/></div>
                <div><div className="flex justify-between text-[10px] text-slate-400 mb-1"><span>Reactivity Intensity</span><span>{state.visualSettings.intensity}x</span></div><input type="range" min="0.5" max="3" step="0.1" value={state.visualSettings.intensity} onChange={(e) => updateVisualSetting('intensity', parseFloat(e.target.value))} className="w-full h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-pink-500"/></div>
             </div>
             ...
             <div className="flex flex-col gap-2 mt-2 p-2 bg-white/5 rounded border border-white/5">
                <div className="flex items-center justify-between text-[10px] text-slate-400">
                    ...
                    <button 
                        onClick={() => updateVisualSetting('cameraShake', !state.visualSettings.cameraShake)}
                        className={`w-8 h-4 rounded-full p-0.5 transition-colors relative ${state.visualSettings.cameraShake ? 'bg-red-500' : 'bg-slate-700'}`}
                    >
                        <div className={`w-3 h-3 bg-white rounded-full shadow-md transform transition-transform ${state.visualSettings.cameraShake ? 'translate-x-4' : 'translate-x-0'}`}></div>
                    </button>
                </div>
                ...
                {state.visualSettings.cameraShake && (
                    <div className="animate-fadeIn">
                       ...
                       <input 
                           ...
                           onChange={(e) => updateVisualSetting('cameraShakeIntensity', parseFloat(e.target.value))}
             ...
             <div className="flex items-center justify-between text-[10px] text-slate-400 mt-2 p-1 bg-white/5 rounded">
                ...
                <button 
                    onClick={() => updateVisualSetting('dynamicBackgroundOpacity', !state.visualSettings.dynamicBackgroundOpacity)}
                ...
                >
                    <div className={`w-3 h-3 bg-white rounded-full shadow-md transform transition-transform ${state.visualSettings.dynamicBackgroundOpacity ? 'translate-x-4' : 'translate-x-0'}`}></div>
                </button>
             </div>
             ...
             <div className="flex items-center justify-between text-[10px] text-slate-400 mt-2 p-1 bg-white/5 rounded">
                ...
                <button 
                    onClick={() => updateVisualSetting('trailsEnabled', !state.visualSettings.trailsEnabled)}
                ...
                >
                    <div className={`w-3 h-3 bg-white rounded-full shadow-md transform transition-transform ${state.visualSettings.trailsEnabled ? 'translate-x-4' : 'translate-x-0'}`}></div>
                </button>
             </div>
          ...
          <div className="space-y-2 pt-4 border-t border-white/10">
             <div className="flex justify-between items-center">
                 <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider">04. Timeline</h3>
                 <button 
                   onClick={() => {
             ...
             {editMode && (
                <div className="p-2 bg-slate-800/50 rounded-lg border border-white/5 space-y-2 animate-fadeIn">
                   ...
                   <div className="flex justify-between items-center text-[10px]">
                      <button onClick={selectAllLyrics} className="text-cyan-400 hover:text-cyan-300 font-bold flex items-center gap-1"><ListBulletIcon className="w-3 h-3" />{selectedLyricIndices.size === state.lyrics.length ? 'Deselect All' : 'Select All'}</button>
             ...
             <div className="h-96 overflow-y-auto pr-2 space-y-1 custom-scrollbar">
                {state.lyrics.map((line, idx) => {
                    const isActive = state.currentTime >= line.startTime && state.currentTime <= line.endTime;
                    ...
                    if (isSelected) {
                        containerClass = 'bg-pink-500/10 border border-pink-500/30 border-l-2 border-l-pink-500';
                    } else if (isActive) {
                        containerClass = 'bg-gradient-to-r from-pink-500/20 to-transparent border-l-2 border-pink-500';
                    } else {
                    ...
                    >
                        ...
                        <div className="flex gap-2 w-full relative z-10">
                            {editMode && (
                                <div className={`mt-1 flex-shrink-0 w-3 h-3 rounded-full border flex items-center justify-center ${selectedLyricIndices.has(idx) ? 'bg-pink-500 border-pink-500' : 'border-slate-600 bg-black/30'}`}>
                                    {selectedLyricIndices.has(idx) && <CheckCircleIcon className="w-3 h-3 text-white" />}
                                </div>
                            )}
                                ...
                                {editMode && isSelected ? (
                                    <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
                                        <div className="flex gap-1 mb-1">
                                            <div className="flex items-center gap-1">
                                                <input type="number" step="0.1" value={line.startTime} onChange={(e) => updateLyric(idx, 'startTime', parseFloat(e.target.value))} className="w-12 bg-black/50 text-cyan-400 p-1 rounded border border-white/10 text-[10px]"/>
                                                <span className="text-slate-600">-</span>
                                                <input type="number" step="0.1" value={line.endTime} onChange={(e) => updateLyric(idx, 'endTime', parseFloat(e.target.value))} className="w-12 bg-black/50 text-cyan-400 p-1 rounded border border-white/10 text-[10px]"/>
                                            </div>
                                            ...
                                            >
                                                ...
                                                {MUSIC_SECTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                                            </select>
                                        </div>

                                        <input type="text" value={line.text} onChange={(e) => updateLyric(idx, 'text', e.target.value)} className="w-full bg-black/50 text-white p-1 rounded border border-white/10 text-[10px]"/>
                                        
                                        {!selectedLyricIndices.has(idx) && line.keyframes && line.keyframes.length > 0 && (
                                             <div className="text-[9px] text-cyan-500 flex items-center gap-1 mt-1 font-mono"><SparklesIcon className="w-3 h-3" /><span>Motion Active</span></div>
                                        )}
                                        ...
                                        <>
                                            <div className="flex flex-col gap-2 mt-2 bg-black/40 p-2 rounded border border-white/5">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-[9px] text-slate-500 uppercase font-bold">Motion Presets</span>
                                                    <button onClick={() => addKeyframe(idx)} className="p-1 text-cyan-400 hover:text-white"><PlusCircleIcon className="w-4 h-4"/></button>
                                                </div>
                                                ...
                                                {line.keyframes && line.keyframes.map((kf, kfIdx) => (
                                                    <div 
                                                        key={kfIdx} 
                                                    ...
                                                    >
                                                        <div className="col-span-4 flex justify-between items-center pb-1 mb-1 border-b border-white/5">
                                                            <div className="flex items-center gap-1">
                                                                <span className="text-slate-500 font-bold">Keyframe {kfIdx + 1}</span>
                                                                {activeKeyframeIndex === kfIdx && <CursorArrowRaysIcon className="w-3 h-3 text-cyan-400 animate-pulse"/>}
                                                            </div>
                                                            <button onClick={() => removeKeyframe(idx, kfIdx)} className="text-red-500 hover:text-red-400"><TrashIcon className="w-3 h-3"/></button>
                                                        </div>

                                                        <div className="col-span-2">
                                                           <label className="text-slate-600 block mb-0.5">Time (0-1)</label>
                                                           <input type="number" step="0.05" min="0" max="1" value={kf.time} onChange={(e) => updateKeyframe(idx, kfIdx, 'time', parseFloat(e.target.value))} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-cyan-300" />
                                                        </div>
                                                        <div className="col-span-2">
                                                           <label className="text-slate-600 block mb-0.5">Ease</label>
                                                           <select value={kf.easing || 'linear'} onChange={(e) => updateKeyframe(idx, kfIdx, 'easing', e.target.value)} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-slate-300">
                                                        ...
                                                        <div className="col-span-4">
                                                           ...
                                                           <div className="flex items-center gap-2">
                                                               <input 
                                                                 ...
                                                                 value={kf.opacity} 
                                                                 onChange={(e) => updateKeyframe(idx, kfIdx, 'opacity', parseFloat(e.target.value))} 
                                                        ...
                                                        <div>
                                                           <label className="text-slate-600 block mb-0.5">Scale</label>
                                                           <input type="number" step="0.1" value={kf.scale} onChange={(e) => updateKeyframe(idx, kfIdx, 'scale', parseFloat(e.target.value))} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-slate-300" />
                                                        </div>
                                                        
                                                        <div className="col-span-1">
                                                            <label className="text-slate-600 block mb-0.5">Rot deg</label>
                                                            <input type="number" step="15" value={kf.rotation} onChange={(e) => updateKeyframe(idx, kfIdx, 'rotation', parseFloat(e.target.value))} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-slate-300" />
                                                        </div>

                                                        <div>
                                                            <label className="text-slate-600 block mb-0.5">X</label>
                                                            <input type="number" step="10" value={kf.x} onChange={(e) => updateKeyframe(idx, kfIdx, 'x', parseFloat(e.target.value))} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-slate-300" />
                                                        </div>
                                                        <div>
                                                            <label className="text-slate-600 block mb-0.5">Y</label>
                                                            <input type="number" step="10" value={kf.y} onChange={(e) => updateKeyframe(idx, kfIdx, 'y', parseFloat(e.target.value))} className="w-full bg-slate-800 border border-white/10 rounded px-1 py-0.5 text-slate-300" />
                                                        </div>
                                                    </div>
                                                ))}
                                    ...
                                    <div className="cursor-pointer w-full">
                                        <div className="flex justify-between items-end mb-0.5">
                                            <span className="text-[10px] text-cyan-500 font-mono">{line.startTime.toFixed(1)}s</span>
                                            {line.section && <span className={`text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded-full bg-black/30 border border-white/5 ${line.section.includes('Chorus') ? 'text-pink-400' : 'text-slate-500'}`}>{line.section}</span>}
                                        </div>
                                        <div className="flex items-center gap-2">
                                            {line.sentimentColor && (
                                                <div className="w-2 h-2 rounded-full" style={{ backgroundColor: line.sentimentColor }}></div>
                                            )}
                                            <div className="text-slate-300 leading-tight truncate relative z-10 font-medium drop-shadow-md">{line.text}</div>
                                        </div>
                                        {line.keyframes && <div className="mt-1 h-0.5 w-full bg-cyan-500/30 rounded"></div>}
                                    </div>
                                )}
                            </div>
                        </div>
      ...
      <div className="flex-1 flex flex-col relative bg-black">
        ...
        <div className="flex-1 flex items-center justify-center p-8 overflow-hidden relative">
            <Visualizer
                ...
                selectedLyricIndex={selectedLyricIndices.size === 1 ? Array.from(selectedLyricIndices)[0] : null}
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-slate-900/80 backdrop-blur border border-white/10 px-6 py-3 rounded-2xl z-20">
                ...
                }} className="text-slate-400 hover:text-white"><ArrowUpTrayIcon className="w-5 h-5 -rotate-90" /></button>
                ...
                >
                    {state.isPlaying ? <PauseIcon className="w-6 h-6" /> : <PlayIcon className="w-6 h-6 ml-1" />}
                </button>
                ...
                <div className="flex flex-col gap-1 w-64">
                    ...
                    <div className="flex justify-between text-[10px] text-slate-400 font-mono">
                        ...
                        <span>{audioElementRef.current?.duration ? new Date(audioElementRef.current.duration * 1000).toISOString().substr(14, 5) : "00:00"}</span>
                    </div>
                </div>
            </div>
        ...
        {chatOpen && (
            <div className="w-80 border-l border-white/10 bg-slate-900/90 backdrop-blur-xl absolute right-0 top-0 bottom-0 z-30 flex flex-col shadow-2xl transition-transform duration-300">
                <div className="p-4 border-b border-white/10 flex justify-between items-center">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">AI Director</h3>
                    <button onClick={() => setChatOpen(false)}><XMarkIcon className="w-5 h-5 text-slate-500" /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar" ref={scrollRef}>
                    {chatMessages.map((msg, i) => (
                        <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                            <div className={`max-w-[85%] p-3 rounded-2xl text-xs leading-relaxed ${msg.role === 'user' ? 'bg-pink-500 text-white rounded-br-none' : 'bg-slate-800 text-slate-300 rounded-bl-none'}`}>
                                {msg.text}
                            </div>
                            <span className="text-[9px] text-slate-600 mt-1">{msg.timestamp.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                        </div>
                    ))}
                    {isProcessing && <div className="flex items-center gap-2 text-xs text-slate-500 animate-pulse"><SparklesIcon className="w-4 h-4" /> Thinking...</div>}
                </div>
                <div className="p-4 border-t border-white/10">
                    <form onSubmit={async (e) => {
                            ...
                            if (response.functionCalls) {
                                for (const call of response.functionCalls) {
                                    if (call.name === 'generateBackgroundImage') {
                                        ...
                                        setChatMessages(prev => [...prev, { role: 'model', text: `Generating background image: ${args.prompt}...`, timestamp: new Date() }]);
                                        const url = await generateBackground(args.prompt, args.aspectRatio || '16:9', args.resolution || '1K');
                                        setState(prev => ({ ...prev, backgroundAsset: { type: 'image', url, prompt: args.prompt } }));
                                        setChatMessages(prev => [...prev, { role: 'model', text: `Background generated!`, timestamp: new Date() }]);
                                    }
                                }
                            }
                        } catch (err) {
                            console.error(err);
                            setChatMessages(prev => [...prev, { role: 'model', text: "Sorry, I encountered an error.", timestamp: new Date() }]);
                        } finally {
                            setIsProcessing(false);
                        }
                    }} className="relative">
                        ...
                        />
                        <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-pink-500 rounded-full text-white hover:bg-pink-600"><PaperAirplaneIcon className="w-4 h-4" /></button>
                    </form>
                </div>
            </div>
        )}
