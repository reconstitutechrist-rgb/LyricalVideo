...
const Visualizer = forwardRef<HTMLCanvasElement, VisualizerProps>(
  (
    ...
    const draw = () => {
      ...
      // 2. Camera Shake Physics (disabled in lyrics-only mode)
      let shakeX = 0;
      let shakeY = 0;
      if (settings.cameraShake && !settings.lyricsOnlyMode) {
        const threshold = 160;
      ...
      if (false) {
        ...
        if (effectiveStyle === VisualStyle.NEON_PULSE) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            const y0 =
              height / 2 +
              Math.sin(wavePhase) * amplitude1 +
              Math.sin(-timeWaveOffset) * amplitude2;
            ctx.moveTo(0, y0);
          ...
          for (let c = 0; c < cloudCount; c++) {
            ...
            const cloudY =
              height * (0.3 + Math.sin(c) * 0.2) + Math.cos(currentTime * 0.15 + c) * 30;
          ...
          for (let layer = 0; layer < 3; layer++) {
            ...
            for (let s = 0; s < starCount; s++) {
              ...
              const brightness =
                0.3 + (2 - layer) * 0.2 + Math.sin(currentTime * 2 + s * 0.1) * 0.2;
      ...
      const renderLyric = (line: LyricLine, opacityFactor: number, isEditingThisLine: boolean) => {
        ...
        const fontSize = (height / 14) * (settings.fontSizeScale || 1.0);
